...
pub struct Cache {
    ...
    total_frequency: u64,
    total_pairs: u64,
    total_extended_pairs: u64,
    distribution: Vec<u64>,
    total_pair_equivalence: f64,
    total_extended_pair_equivalence: f64,
    total_duplication: u64,
    total_fingering: [u64; 8],
    total_levels: Vec<u64>,
    tiers_duplication: Vec<u64>,
    tiers_levels: Vec<Vec<u64>>,
    tiers_fingering: Vec<[u64; 8]>,
    max_index: u64,
    segment: u64,
...
impl Cache {
    pub fn new(
        ...
        let tiers_fingering = vec![[0_u64; 8]; ntier];
    ...
    pub fn accumulate(
        ...
        if partial_weights.pair_equivalence.is_some() {
            ...
            while code > self.radix {
                ...
                self.total_pair_equivalence +=
                    objective.pair_equivalence[partial_code] * frequency as f64;
                code /= self.segment;
            }
        }
        // 3. 词间当量
        if partial_weights.extended_pair_equivalence.is_some() {
            let transitions = &objective.encoder.transition_matrix[index];
            let last_char = code / self.radix.pow(length as u32 - 1);
            for (i, weight) in transitions {
                let next_char = objective.encoder.buffer[*i].full.code % radix;
                let combination = last_char + next_char * radix;
                let equivalence = objective.pair_equivalence[combination as usize];
                self.total_extended_pair_equivalence += equivalence * *weight as f64;
                self.total_extended_pairs += *weight;
            }
        }
        // 4. 差指法
        if let Some(fingering) = &partial_weights.fingering {
            let mut code = code;
            while code > radix {
                let label = objective.fingering_types[(code % self.max_index) as usize];
                for (i, weight) in fingering.iter().enumerate() {
                    if weight.is_some() {
                        self.total_fingering[i] += frequency * label[i] as u64;
                    }
                }
                code /= self.segment;
            }
        ...
        if let Some(tiers) = &partial_weights.tiers {
            for (itier, tier) in tiers.iter().enumerate() {
                ...
                // 1. 重码
                if duplicate {
                    self.tiers_duplication[itier] += 1;
                }
                // 2. 简码
                if let Some(levels) = &tier.levels {
                    for (ilevel, level) in levels.iter().enumerate() {
                        if level.length == length as usize {
                            self.tiers_levels[itier][ilevel] += 1;
                        }
                    }
                }
                // 3. 差指法
                if let Some(fingering) = &tier.fingering {
                    ...
                    while code > radix {
                        ...
                        for (i, weight) in fingering.iter().enumerate() {
                            if weight.is_some() {
                                self.tiers_fingering[itier][i] += label[i] as u64;
                            }
                        }
                        code /= self.segment;
                    }
                    ...
                    tiers[itier].levels = Some(
                        level_weight
                            ...
                            .map(|(i, v)| LevelMetricUniform {
                                length: v.length,
                                frequency: self.tiers_levels[itier][i],
                            })
                            .collect(),
                    );
                }
