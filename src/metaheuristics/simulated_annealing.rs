...
pub fn solve<T: Clone, M: Clone>(problem: &mut dyn Metaheuristics<T, M>, parameters: Parameters) -> T {
    ...
    let Parameters { t_max, t_min, steps } = parameters;
...
fn trial_run<T: Clone, M: Clone>(problem: &mut dyn Metaheuristics<T, M>, from: T, temperature: f64, steps: usize) -> (T, f64, f64) {
...
pub fn autosolve<T: Clone, M: Clone>(problem: &mut dyn Metaheuristics<T, M>, duration: Duration) -> T {
    let batch = 1000;
    println!("开始寻找参数……");
    ...
    while accept_rate > 0.98 {
        ...
        (candidate, accept_rate, improve_rate) = trial_run(problem, candidate, temperature, batch);
        total_steps += batch;
        println!("若温度为 {:.2e}，接受率为 {:.2}%", temperature, accept_rate * 100.0);
    }
    while accept_rate < 0.98 {
        temperature *= 2.0;
        (candidate, accept_rate, improve_rate) = trial_run(problem, candidate, temperature, batch);
    ...
    println!("接受率已符合标准，体系最高温度估计为：t_max = {:.2e}", t_max);
    ...
    while improve_rate > 0.005 {
        ...
        total_steps += batch;
        println!("若温度为 {:.2e}，改进率为 {:.2}%", temperature, improve_rate * 100.0);
    }
    let t_min = temperature;
    println!("改进率已符合标准，体系最低温度估计为：t_min = {:.2e}", t_min);
    let elapsed = start.elapsed();
    let steps = total_steps * duration.as_millis() as usize / elapsed.as_millis() as usize;
    println!("参数寻找完成，将在 {} 分钟内用 {} 步为您优化……", duration.as_secs() / 60, steps);
    let parameters = Parameters { t_max, t_min, steps };
